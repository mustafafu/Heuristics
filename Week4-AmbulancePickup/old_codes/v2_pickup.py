...
from multiprocessing import Pool, freeze_support, cpu_count,TimeoutError,Process
...
def main():
    ...
    parser.add_argument('--input', type=str, default='data3',
                        help='What is the data file (string)?')
    parser.add_argument('--output', type=str, default='out3',
...
if __name__ == '__main__':
    ...
    # for hospital_idx in range(num_hospitals):
    #     number_of_ambulance = best_match[hospital_idx]
    #     hospital_routing = hospital_routings[hospital_idx]
    #     for ambulance in range(number_of_ambulance):
    #         routes = hospital_routing[ambulance][0]
    #         for route_w_minus in routes:
    #             this_route = route_w_minus[np.where(route_w_minus > -1)[0]]
    #             my_string = 'Ambulance_{}: {}: ({},{})'.format(ambulance,hospital_idx + 1, hospitals[hospital_idx, 0],hospitals[hospital_idx, 1])
    #             for patient in this_route[1:-1]:
    #                 my_string = my_string + ', {}: ({},{},{})'.format(patient + 1, patients[patient, 0],
    #                                                                   patients[patient, 1], patients[patient, 2])
    #             my_string = my_string + ', {}: ({},{})'.format(hospital_idx + 1, hospitals[hospital_idx, 0],
    #                                                            hospitals[hospital_idx, 1])
    #             # print(my_string)
    #             fin.write(my_string + '\n')
    #
    # fin.close()
    ...
    # hospital_idx = 0
    # for hospital_idx in range(num_hospitals):
    #     hospital_routing = route_hospital(hospital_idx, patient_hospital_match, distances, hospital_start_idxs, patients, max_ambulance)
    #     print('Scheduled hospital:{}, total time elapsed: {}'.format(hospital_idx,time.time()-t0))
    #     print(hospital_routing)
    #     print('-'*100)
    ...
    #     each_ambulance_route.append((selected_routes,best_value))
    #     print('---------Ambulance {}---------'.format(this_ambulance))
    #     for route_idx in selected_routes:
    #         chosen_route_w_minus = routes[route_idx]
    #         chosen_route = chosen_route_w_minus[np.where(chosen_route_w_minus > -1)[0]]
    ...
    #     this_route_deadlines = route_deadlines[selected_routes]
    #     this_route_distances = route_distances[selected_routes]
    #     best_routes,best_value = greedy_scheduler_forwards(current_time, this_routes,this_route_deadlines, this_route_distances, scheduled_patients)
        # best_routes = selected_routes[best_routes]
    #     for chosen_route_w_minus in this_routes[best_routes]:
    #         chosen_route = chosen_route_w_minus[np.where(chosen_route_w_minus > -1)[0]]
    #         # print(chosen_route)
    #         for patient in chosen_route[1:-1]:
    #             scheduled_patients.add(patient)
    #     each_ambulance_route.append((selected_routes[best_routes],best_value))
    #     print('---------Ambulance {}---------'.format(this_ambulance))
    #     for route_idx in selected_routes[best_routes]:
    #         chosen_route_w_minus = routes[route_idx]
    #         chosen_route = chosen_route_w_minus[np.where(chosen_route_w_minus > -1)[0]]
    #         print('Route: {}, How long: {}, Deadline:{}'.format(chosen_route,route_distances[route_idx],route_deadlines[route_idx]))
    #     this_ambulance += 1
    ...
    # this_ambulance = 1
    # while this_ambulance <= hospitals[hospital_idx,2]:
    #     (selected_routes,best_value) = method_backwards(routes, route_deadlines, route_distances, how_many_patients, scheduled_patients)
    #     each_ambulance_route.append((selected_routes,best_value))
    #     print('---------Ambulance {}---------'.format(this_ambulance))
    #     for route_idx in selected_routes:
    #         chosen_route_w_minus = routes[route_idx]
...
# kmedians = KMedians(k=num_hospitals)
# kmedians.fit(patients[:,:2])
# hospital_locations = kmedians.cluster_centers_
# hospitals[:, :2] = hospital_locations
# #
# all_locations_matrix = np.zeros([V_c+W+W,2],dtype='int')
# all_locations_matrix[:V_c,:] = patients[:,:2]
...
#     distances[i][i] = 0
# patient_hospital_distance = distances[patient_idxs.reshape(-1, 1), hospital_start_idxs.reshape(1, -1)]
# print('Kmedians distances sum {}'.format(patient_hospital_distance.min(axis=1).sum()))
    ...
    # fp_sorted = np.argsort(route_distances / how_many_patients)
    # scheduled_patients = set()
    # selected_routes = []
    # route_properties = np.zeros([this_hospital_patients_idx.shape[0],4]) #value,deadline,distance,deadline-distance
    # route_counter = 0
    # for index in fp_sorted:
    #     candidate_route_w_minus = routes[index]
    #     candidate_route = candidate_route_w_minus[np.where(candidate_route_w_minus > -1)[0]]
    #     if len(scheduled_patients.intersection(candidate_route)) == 0:
    #         selected_routes.append(index)
    #         # print(candidate_route)
    #         route_properties[route_counter,:] = np.array([candidate_route.shape[0] - 2, route_deadlines[index],route_distances[index], route_deadlines[index] - route_distances[index]])
    #         route_counter += 1
    #         for patient in candidate_route[1:-1]:
    #             scheduled_patients.add(patient)
    #     else:
    #         continue
    # route_properties = route_properties[:route_counter]
    # sorting_idx = np.argsort(route_properties[:, -1])
    # selected_routes = np.array(selected_routes)
    ...
    # this_hospital_patients_idx = np.where(patient_hospital_match == hospital_idx)[0]
    # n_patients = this_hospital_patients_idx.shape[0]
    # k_choose = np.arange(1,5)
    # # route_possibility_count = int(comb(n_patients,k_choose).sum())
    # route_possibility_count = n_patients * (1 + (n_patients-1) *(1 + (n_patients-2)*(1+(n_patients-3))))
    # routes = -1*np.ones([route_possibility_count ,6],dtype='int16')
    # route_deadlines = -1*np.ones(route_possibility_count ,dtype='int16')
    # route_distances = -1 * np.ones(route_possibility_count, dtype='int16')
    ...
    # res.sort(axis=1)
    # res = np.unique(res,axis=0)
    # this_routes, this_route_deadlines, this_route_distances = return_shortest_route_w_tsp_fixed_hospital(res, distances,hospital_start_idxs[hospital_idx], patients)
    # n_routes = this_routes.shape[0]
    # routes[routes_idx:routes_idx + n_routes, :number_of_patients + 2] = this_routes
    # route_deadlines[routes_idx:routes_idx + n_routes] = this_route_deadlines
    # route_distances[routes_idx:routes_idx + n_routes] = this_route_distances
    ...
    #     # print(number_of_patients)
    #     feasible_patients = this_routes[:,1:-1]
    #     additional_patients_combos = [(*y[0], y[1]) for y in itertools.product(feasible_patients.tolist(), this_hospital_patients_idx)]
    #     additional_patients_combos = [combo for combo in additional_patients_combos if len(combo) == len(set(combo))]
    #     t0 = time.time()
    #     res = np.array(additional_patients_combos, dtype=np.int16)
    #     res.sort(axis=1)
